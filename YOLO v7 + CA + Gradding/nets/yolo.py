...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False):
        ...
        self.rep_conv_1_1 = conv(transition_channels * 4, transition_channels * 8, 3, 1)
        # 40, 40, 256 => 40, 40, 512
        self.rep_conv_2_1 = conv(transition_channels * 8, transition_channels * 16, 3, 1)
        # 20, 20, 512 => 20, 20, 1024
        self.rep_conv_3_1 = conv(transition_channels * 16, transition_channels * 32, 3, 1)


        # 4 + 1 + num_classes
        # 80, 80, 256 => 80, 80, 3 * 25 (4 + 1 + 20) & 85 (4 + 1 + 80)
        self.yolo_head_P3 = nn.Conv2d(transition_channels * 8, len(anchors_mask[2]) * (5 + 3), 1)
        # 40, 40, 512 => 40, 40, 3 * 25 & 85
        self.yolo_head_P4 = nn.Conv2d(transition_channels * 16, len(anchors_mask[1]) * (5 + 3), 1)
        # 20, 20, 512 => 20, 20, 3 * 25 & 85
        self.yolo_head_P5 = nn.Conv2d(transition_channels * 32, len(anchors_mask[0]) * (5 + 3), 1)

        self.yolo_head_P3_1 = nn.Conv2d(transition_channels * 8, 3 * 3, 1)
        # 40, 40, 512 => 40, 40, 3 * 25 & 85
        self.yolo_head_P4_1 = nn.Conv2d(transition_channels * 16, 3 * 3, 1)
        # 20, 20, 512 => 20, 20, 3 * 25 & 85
        self.yolo_head_P5_1 = nn.Conv2d(transition_channels * 32, 3 * 3, 1)

        self.end_1 = nn.Conv2d(33, 42, 1)
        self.end_2 = nn.Conv2d(33, 42, 1)
        self.end_3 = nn.Conv2d(33, 42, 1)
    ...
    def forward(self, x):
        ...
        P3_1 = self.rep_conv_1(P3)
        P4_1 = self.rep_conv_2(P4)
        P5_1 = self.rep_conv_3(P5)

        P3_2 = self.rep_conv_1_1(P3)
        P4_2 = self.rep_conv_2_1(P4)
        P5_2 = self.rep_conv_3_1(P5)



        out2_1 = self.yolo_head_P3(P3_1)
        out1_1 = self.yolo_head_P4(P4_1)
        out0_1 = self.yolo_head_P5(P5_1)

        out2_2 = self.yolo_head_P3_1(P3_2)
        out1_2 = self.yolo_head_P4_1(P4_2)
        out0_2 = self.yolo_head_P5_1(P5_2)


        out2 = torch.cat([out2_1, out2_2], 1)
        out1 = torch.cat([out1_1, out1_2], 1)
        out0 = torch.cat([out0_1, out0_2], 1)
        
        out2 = self.end_1(out2)
        out1 = self.end_2(out1)
        out0 = self.end_3(out0)
...
if __name__ == '__main__':
    ...
    # 33 = 3 * (4 + 1 + 3(病害种类) + 3(病害分级))
    net = YoloBody(anchors_mask, num_classes, phi, False)
    output = net(torch.randn(1, 3, 640, 640))
    # P5 20, 20, 33
